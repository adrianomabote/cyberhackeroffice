 ...
 * 
 * COMANDOS:
 * - Para parar: stopCyberCapture()
 * - Para reiniciar: startCyberCapture()
 */

(function () {
  // ========== CONFIGURAÇÕES ==========
  const API_URL = 'https://e1f022d7-32c0-4ec8-b553-24a963ff6795-00-332y5p5zgrxlq.picard.replit.dev/api/velas/cyber';
  ...
  async function capturarMultiplicador() {
    try {
      ...
      let multiplicadorTexto =
        multiplicadorElement.textContent || multiplicadorElement.innerText;
      multiplicadorTexto = multiplicadorTexto.replace(/[^\d.,]/g, '');
      multiplicadorTexto = multiplicadorTexto.replace(',', '.');
      ...
      console.log(
        `🎯 CYBER HACKER: Capturado ${multiplicador.toFixed(
          2
        )}x (#${totalCapturados})`
      );
      ...
      if (response.ok) {
        ...
        errosConsecutivos = 0;
        console.log(
          `✅ CYBER HACKER: Enviado ${multiplicador.toFixed(
            2
          )}x para servidor (Total: ${totalEnviados})`
        );
      } else {
        errosConsecutivos++;
        console.error(
          `❌ CYBER HACKER: Erro ao enviar ${multiplicador.toFixed(
            2
        ...
        if (errosConsecutivos >= 5) {
          console.error(
            '🚨 CYBER HACKER: Muitos erros consecutivos. Verifique a conexão com o servidor.'
          );
        }
      }
    } catch (error) {
  ...
  function pararCaptura() {
    ...
    console.log(
      `   - Último multiplicador: ${
        ultimoMultiplicador ? ultimoMultiplicador.toFixed(2) : 'N/A'
      }x`
    );
  }
